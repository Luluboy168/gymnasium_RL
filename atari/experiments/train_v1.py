...
def build_q_network(input_shape, num_actions):
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.00025), loss='huber')
    return model
...
for episode in range(num_episodes):
    ...
    obs, _ = envs.reset()
    obs = np.stack([preprocess_observation(o) for o in obs])  # (8, 84, 84)
    ...
    while not np.all(done_flags):
        ...
        next_obs = np.stack([preprocess_observation(o) for o in next_obs])
